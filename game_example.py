...
class game:
    ...
    def create_food(self, ball):
        ...
        for i in range(0, len(self.foodX)):
            self.canvas.create_rectangle(self.foodX[i], self.foodY[i], self.foodX[i] + 10, self.foodY[i] + 10,
                                         fill="blue")
        for i in range(0, len(self.foodX)):
            if len(self.canvas.find_overlapping(self.foodX[i], self.foodY[i], self.foodX[i] + 10,
                                                self.foodY[i] + 10)) is not 1:
                if ball in self.canvas.find_overlapping(self.foodX[i], self.foodY[i], self.foodX[i] + 10,
                                                        self.foodY[i] + 10):
                    self.point += 100
                    self.size += 0.5
                    self.foodX.pop(i)
    ...
    def create_trap(self, ball):
        ...
        for i in range(0, len(self.trapX)):
            self.canvas.create_rectangle(self.trapX[i], self.trapY[i], self.trapX[i] + 10, self.trapY[i] + 10,
                                         fill="red")
        for i in range(0, len(self.trapX)):
            if len(self.canvas.find_overlapping(self.trapX[i], self.trapY[i], self.trapX[i] + 10,
                                                self.trapY[i] + 10)) is not 1:
                if ball in self.canvas.find_overlapping(self.trapX[i], self.trapY[i], self.trapX[i] + 10,
                                                        self.trapY[i] + 10):
                    self.point -= 50
                    self.size -= 1
                    self.trapX.pop(i)
    ...
    def create_powersize(self, ball):
        ...
        for i in range(0, len(self.powerupX[0])):
            self.canvas.create_rectangle(self.powerupX[0][i], self.powerupY[0][i], self.powerupX[0][i] + 10,
                                         self.powerupY[0][i] + 10, fill="yellow")
        for i in range(0, len(self.powerupX[0])):
            if len(self.canvas.find_overlapping(self.powerupX[0][i], self.powerupY[0][i], self.powerupX[0][i] + 10,
                                                self.powerupY[0][i] + 10)) is not 1:
                if ball in self.canvas.find_overlapping(self.powerupX[0][i], self.powerupY[0][i],
                                                        self.powerupX[0][i] + 10, self.powerupY[0][i] + 10):
                    self.point += 150
                    self.size += 2
    ...
    def create_powercoin(self, ball):
        ...
        for i in range(0, len(self.powerupX[1])):
            self.canvas.create_rectangle(self.powerupX[1][i], self.powerupY[1][i], self.powerupX[1][i] + 10,
                                         self.powerupY[1][i] + 10, fill="yellow")
        for i in range(0, len(self.powerupX[1])):
            if len(self.canvas.find_overlapping(self.powerupX[1][i], self.powerupY[1][i], self.powerupX[1][i] + 10,
                                                self.powerupY[1][i] + 10)) is not 1:
                if ball in self.canvas.find_overlapping(self.powerupX[1][i], self.powerupY[1][i],
                                                        self.powerupX[1][i] + 10, self.powerupY[1][i] + 10):
                    self.point += 500
                    self.size -= 0.5
    ...
    def paint(self):
        ...
        if self.time // 100 <= 60:
            if 10 * self.size > 0:
                self.TEXT = "Welcome to tkinter"
                ball = self.canvas.create_oval(self.x - 10 * self.size, self.y - 10 * self.size,
                                               self.x + 10 * self.size, self.y + 10 * self.size, fill="white")
                self.create_food(ball)
                self.create_trap(ball)
